...
export async function deleteStudent(studentId: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (dbErr || !authRes.success) {
      return { success: false, error: dbErr?.message || authRes.error }
    }
    return { success: true }
  } catch (e: any) {
...
fs.writeFileSync(path.join('d:/anfaz/coding/calam wing/src/utils/db.ts'), content);
console.log('db.ts rewritten successfully!');
